...
const FALLBACK_BEFORE = 'https://images.unsplash.com/photo-1600607687920-4e2a09cf159d?q=80&w=1200&auto=format&fit=crop'
const FALLBACK_AFTER = '/images/roof-install-after.jpg'
...
export default function BeforeAfterSlider() {
  ...
  const projectId = settings?.homeBeforeAfter?._id || settings?.homeBeforeAfter

  // Fetch the selected project to get its before/after images
  const { data: projectData } = useResourceItem('projects', projectId || null)
  const project = projectData?.data

  const before = project?.beforeImages?.[0]?.url ?? FALLBACK_BEFORE
  const after = project?.afterImages?.[0]?.url ?? FALLBACK_AFTER
    ...
    <section className="bg-ink-50 py-24 dark:bg-ink-900/30 md:py-32">
      <Container>
        <SectionHeading
          ...
          description={
            title
              ? `Drag the slider to compare before and after — ${title}.`
              : 'Drag the slider to compare a real before-and-after from a recent residential re-roof.'
          }
          align="center"
          className="mx-auto"
        />
