...
export const createService = asyncHandler(async (req, res) => {
  ...
  // See parseJsonField.js — multipart requests deliver `pricing` as a
  // JSON string, and Mongoose silently drops it if left unparsed.
  ...
  if (files.gallery?.length) {
    const uploaded = await Promise.all(
      files.gallery.map((f) => uploadBuffer(f.buffer, { folder: 'roofing-cms/services/gallery' }))
    )
...
/**
 * PATCH /services/reorder  body: { order: [{ id, order }] }
 */
